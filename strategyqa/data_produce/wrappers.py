...
DATA_DIR = work_dir + "/data_produce/data"
...
class StrategyQAWrapper(gym.Wrapper):
  def __init__(self, env, split):
    ...
    data_path = f"{DATA_DIR}/{STRATEGYQA_SPLIT_FILE[split]}"
    with open(data_path, "r") as json_file:
      json_list = list(json_file)
    ...
    for json_str in json_list:
      json_str = json.loads(json_str)
      label = 'true' if json_str["answer"] else 'false'
      claim = json_str["question"]
      data.append((claim, label))
